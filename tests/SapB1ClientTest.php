...
describe('main', function () {
    ...
    it('can use facade to make get requests using value macro', function () {
        $response = SapB1::get('Items');
...
describe('QueryBuilder', function () {
    ...
    it('can create a query builder with query method', function () {
        Http::fake([
            '*Login*' => Http::response(['SessionId' => 'mock_session_id'], 200, ['Set-Cookie' => 'B1SESSION=mock_session_cookie;']),
        ]);

        $client = new SapB1Client;
        $queryBuilder = $client->query('BusinessPartners');

        expect($queryBuilder)->toBeInstanceOf(\BlockshiftNetwork\SapB1Client\QueryBuilder::class);
    });

    it('query builder returns fluent interface', function () {
        Http::fake([
            '*Login*' => Http::response(['SessionId' => 'mock_session_id'], 200, ['Set-Cookie' => 'B1SESSION=mock_session_cookie;']),
        ]);

        $client = new SapB1Client;
        $queryBuilder = $client->query('Items')
            ->select('ItemCode', 'ItemName')
            ->where('ItemCode', '=', 'A001')
            ->top(10);
    ...
    it('can execute query with run method', function () {
        Http::fake([
            '*Login*' => Http::response(['SessionId' => 'mock_session_id'], 200, ['Set-Cookie' => 'B1SESSION=mock_session_cookie;']),
            '*Items*' => Http::response(['value' => [['ItemCode' => 'A001'], ['ItemCode' => 'A002']]], 200),
        ]);
    ...
    it('can execute query with get method alias', function () {
        Http::fake([
            '*Login*' => Http::response(['SessionId' => 'mock_session_id'], 200, ['Set-Cookie' => 'B1SESSION=mock_session_cookie;']),
            '*Items*' => Http::response(['value' => [['ItemCode' => 'A001'], ['ItemCode' => 'A002']]], 200),
        ]);
    ...
    it('can use value macro with query builder response', function () {
        Http::fake([
            '*Login*' => Http::response(['SessionId' => 'mock_session_id'], 200, ['Set-Cookie' => 'B1SESSION=mock_session_cookie;']),
            '*Items*' => Http::response(['value' => [['ItemCode' => 'A001'], ['ItemCode' => 'A002']]], 200),
        ]);
        ...
        $client = new SapB1Client;
        $response = $client->query('Items')->top(2)->run();

        expect($response->value())->toBe([['ItemCode' => 'A001'], ['ItemCode' => 'A002']]);
    });

    it('facade can use query builder', function () {
        Http::fake([
            '*Login*' => Http::response(['SessionId' => 'mock_session_id'], 200, ['Set-Cookie' => 'B1SESSION=mock_session_cookie;']),
            '*Items*' => Http::response(['value' => [['ItemCode' => 'A001'], ['ItemCode' => 'A002']]], 200),
        ]);
    ...
    it('query builder supports all filter methods', function () {
        Http::fake([
            '*Login*' => Http::response(['SessionId' => 'mock_session_id'], 200, ['Set-Cookie' => 'B1SESSION=mock_session_cookie;']),
            '*BusinessPartners*' => Http::response(['value' => [['CardCode' => 'C001', 'CardName' => 'Acme Inc.']]], 200),
        ]);

        $client = new SapB1Client;
        $response = $client->query('BusinessPartners')
            ->select('CardCode', 'CardName', 'Balance')
            ->where('CardType', '=', 'cCustomer')
            ->orWhere('CardName', 'contains', 'Acme')
            ->where('Balance', '>', 0)
        ...
        expect($response->successful())->toBeTrue();
        expect($response->value())->toBeArray();
    });

    it('query builder sends correct odata parameters', function () {
        Http::fake(function ($request) {
            if (str_contains($request->url(), 'Login')) {
                return Http::response(['SessionId' => 'mock_session_id'], 200, ['Set-Cookie' => 'B1SESSION=mock_session_cookie;']);
            }
        ...
        Http::assertSent(function ($request) {
            ...
            return str_contains($url, '%24select=ItemCode%2CItemName')
                && str_contains($url, '%24filter=ItemCode+eq+%27A001%27')
                && str_contains($url, '%24orderby=ItemCode+asc')
                && str_contains($url, '%24top=10')
                && str_contains($url, '%24skip=5');
        });
    });

    it('query builder supports between filter', function () {
        Http::fake(function ($request) {
            if (str_contains($request->url(), 'Login')) {
                return Http::response(['SessionId' => 'mock_session_id'], 200, ['Set-Cookie' => 'B1SESSION=mock_session_cookie;']);
            }
        ...
        Http::assertSent(function ($request) {
            ...
            return str_contains(urldecode($request->url()), 'CreateDate ge')
                && str_contains(urldecode($request->url()), 'CreateDate le');
        });
    });

    it('query builder can chain multiple where conditions', function () {
        Http::fake([
            '*Login*' => Http::response(['SessionId' => 'mock_session_id'], 200, ['Set-Cookie' => 'B1SESSION=mock_session_cookie;']),
            '*BusinessPartners*' => Http::response(['value' => [['CardCode' => 'C001', 'CardName' => 'Acme Inc.']]], 200),
        ]);
    ...
    it('query builder works with value default parameter', function () {
        Http::fake([
            '*Login*' => Http::response(['SessionId' => 'mock_session_id'], 200, ['Set-Cookie' => 'B1SESSION=mock_session_cookie;']),
            '*Empty*' => Http::response(['odata.metadata' => 'something'], 200),
        ]);

        $client = new SapB1Client;
        $response = $client->query('Empty')->run();
